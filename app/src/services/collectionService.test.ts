...
describe('collectionService', () => {
  it('maps record input to the actual collection RPC contract', async () => {
    rpc.mockResolvedValueOnce({ data: 'collection-id', error: null })
    ...
    expect(rpc).toHaveBeenCalledWith('mizan_record_collection', expect.objectContaining({
  ...
  it('maps void requests', async () => {
    ...
    expect(rpc).toHaveBeenCalledWith('mizan_void_collection', { p_collection_id: 'collection-id', p_reason: 'تصحيح', p_operation_id: 'operation-id' })
  })
})
