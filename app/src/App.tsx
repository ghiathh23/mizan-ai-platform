...
export default function App() {
  ...
  async function createReading() {
    ...
    await run(async () => {
      const created = await mizanService.createReading({ project_id: projectId, meter_id: meterId, reading_at: new Date(`${readingDate}T00:00:00Z`).toISOString(), extracted_value: parsed.value, evidence_id: evidenceId || null })
  ...
  async function validateAndBill() {
    ...
    await run(async () => {
      await mizanService.validateReading(selectedReading.id, parsed.value as number)
  ...
  return <main className="shell" dir="rtl">
    ...
    <section className="panel"><h2>سياق المشروع</h2><label>المشروع<select value={projectId} onChange={(event) => { setProjectId(event.target.value); setMeterId(''); setReadingId(''); setBilling(null); setEvidenceId('') }}><option value="">اختر مشروعًا</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select></label><p className="hint">تُحمّل المشاريع من Supabase ولا يوجد project ID ثابت في الواجهة.</p></section>
